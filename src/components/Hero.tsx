...
const AnimatedCounter = ({
  ...
  return <div ref={ref} className="text-2xl lg:text-3xl font-bold text-foreground animate-count-up">
      {displayValue}
    </div>;
};
const Hero = () => {
  ...
  const fetchStats = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('hero_stats').select('*').order('sort_order', {
        ascending: true
      });
  ...
  return <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0">
        <img src={heroImage} alt="Professional Photography" className="w-full h-full object-cover scale-105 animate-[float_20s_ease-in-out_infinite]" />
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-accent/5 rounded-full blur-3xl animate-float" style={{
        animationDelay: '2s'
      }} />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          ...
          <div className="text-center lg:text-left">
            ...
            <p className="text-lg sm:text-xl text-muted-foreground mb-8 max-w-xl mx-auto lg:mx-0 animate-fade-in" style={{
            animationDelay: '0.2s'
          }}>
            ...
            {/* CTA Buttons */}
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mb-12 animate-fade-in" style={{
            animationDelay: '0.3s'
          }}>
            ...
            {/* Stats */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 animate-fade-in" style={{
            animationDelay: '0.4s'
          }}>
              {loading ? Array.from({
              length: 4
            }).map((_, i) => <div key={i} className="text-center p-4 rounded-2xl bg-card/50 backdrop-blur-sm border border-border/50">
                  ...
                  </div>) : heroStats.map((stat, index) => {
              const Icon = getIcon(stat.icon_name);
              return <div key={stat.id} className="text-center group p-4 rounded-2xl bg-card/50 backdrop-blur-sm border border-border/50 hover:border-primary/30 hover:bg-card/80 transition-all duration-300 hover:-translate-y-1 hover:shadow-glow" style={{
